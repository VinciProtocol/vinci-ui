const locale = {
  title: 'NFT Airdrop',
  eligible: {
    title: '🎉 Congrats! You are eligible for the airdrop.',
    subTitle: 'Simply connect your participating address to claim it.',
    willReceive: 'will receive',
    NFT: {
      actions: {
        comingSoon: 'Coming Soon',
        claim: 'Claim',
        claimed: 'Claimed',
        tip: {
          switchAccount: 'Please switch account to claim.',
        },
      },
    },
  },
  loading: {
    title: 'Loading',
  },
  notEligible: {
    title: 'Sorry, you are not eligible for Vinci NFT now.',
    subTitle: {
      1: 'Please view the Eligibility Criteria carefully.',
      2: 'The best way to improve your odds is to get involved.',
    },
    warn: {
      1: 'We are now receiving a surprising amount of whitelist request.',
      2: 'If you have finished the tasks but couldn’t claim  the airdrop reward, please be patient to be whitelisted.',
    },
    actions: {
      switchAccount: 'Check another address',
      borrow: 'Borrow',
      lend: 'Lend',
    },
  },
  tip: {
    warn: {
      1: 'We are now receiving a surprising amount of whitelist request.',
      2: 'If you have finished the tasks but couldn’t claim  the airdrop reward, please be patient.',
    },
  },
  criteria: {
    title: 'Eligibility Criteria',
    subTitle: {
      1: 'The Leonardo da Vinci NFT Drop will reward whoever joins in Vinci Protocol communities and participate in this community initiative to help us create a social movement.',
      2: 'To be eligible, you must enter <contest>the Contest</contest> and make your support known to the public.',
    },
    btn: {
      learnMore: 'Learn More',
    },
  },
  switchAccountDialog: {
    title: 'Paste Address',
    address: 'address',
    helperText: 'Invalid address',
    actions: {
      checkEligibilty: 'Check Eligibilty',
      cancel: 'Cancel',
    },
  },
}

export default locale
