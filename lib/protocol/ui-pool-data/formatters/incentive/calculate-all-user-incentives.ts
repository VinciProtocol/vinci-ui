...
import type { CalculateUserReserveIncentivesResponse } from './calculate-user-reserve-incentives'
...
export function calculateAllUserIncentives({
  ...
  const rewards = userReserveIncentives
    .map((userReserveIncentive: UserReserveIncentiveDataHumanizedResponse) => {
      const reserve: ReserveIncentiveWithFeedsResponse | undefined = reserveIncentives.find(
        (reserve: ReserveIncentiveWithFeedsResponse) => reserve.underlyingAsset === userReserveIncentive.underlyingAsset
      )
      const userReserve: UserReserveCalculationData | undefined = userReserves.find(
        (reserve: UserReserveCalculationData) => reserve.underlyingAsset === userReserveIncentive.underlyingAsset
      )
      if (reserve) {
        ...
        return [
          {
            ...
            incentiveController: userReserveIncentive.aTokenIncentivesUserData.incentiveControllerAddress,
          ...
          },
          {
            ...
            incentiveController: userReserveIncentive.vTokenIncentivesUserData.incentiveControllerAddress,
          ...
          },
          {
            ...
            incentiveController: userReserveIncentive.sTokenIncentivesUserData.incentiveControllerAddress,
  ...
  return rewards.reduce<UserIncentiveDict>((acc: UserIncentiveDict, reward: RewardCalculation) => {
    ...
    if (reward.accruedRewards.gt(0)) {
      acc[reward.incentiveController].claimableRewards = acc[reward.incentiveController].claimableRewards.plus(
