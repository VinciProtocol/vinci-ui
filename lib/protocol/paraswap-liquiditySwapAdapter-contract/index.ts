...
import type { IParaSwapLiquiditySwapAdapter } from './typechain/IParaSwapLiquiditySwapAdapter'
    ...
    const txCallback: () => Promise<transactionType> = this.generateTxCallback({
      rawTxMethod: async () =>
        liquiditySwapContract.populateTransaction.swapAndDeposit(
          ...
          amountToSwap,
          minAmountToReceive,
          swapAll ? augustusFromAmountOffsetFromCalldata(swapCallData as string) : 0,
          swapCallData,
          augustus,
          permitParams
        ),
